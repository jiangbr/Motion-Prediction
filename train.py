    ...
    def __getitem__(self, index: int) -> Tuple:
            ...
            if status[i, 2] == 1:
                ...
                if status[i, 3] == 1 or status[i, 4] == 1:
                    ...
                    if (status[i, 0] >= min_hist and status[i, 1] >= min_fut) \
                        ...
                        # calculate the movement in the history frames
                        startframe = int(100-status[i, 0])
                        startpos = hist_origin[i][startframe, :].reshape(1, 2)
                        presentpos = hist_origin[i][-1, :].reshape(1, 2)
                        move = np.sqrt(np.sum(np.square(presentpos - startpos), axis=1))
                        # extract agents that move enough distance
                        if move > 1:
        ...
        if hist.shape[1] <= 30:
            ...
            for i in range(hist.shape[1]):
                ...
                distance30 = np.sort(distance)[30]
...
def train(rank: int, args: Any) -> None:
    ...
    # train_data = train_dataset[q]
    # hand_data = hand_dataset[1000]
